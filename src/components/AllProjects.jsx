...
// Global video preview config so quality can be controlled in one place.
      ...
      {!hideDetails && (
        <div className="project-info">
          ...
          <div className="project-technologies">
            ...
            {project.id === 13 && (
              <button type="button" className="project-featured-tag" onClick={() => onFeaturedClick && onFeaturedClick(project)} aria-label="Open featured video">Featured</button>
            )}
            {project.isWIP && (
...
const projects = [
  ...
  },
  {
    id: 3,
    title: 'Pitaka',
    description: 'A publicly available personal finance web app for tracking income, expenses, savings, wallets, and lendings with CSV export and realtime Firestore sync. Built around a clean, accessible dashboard and compact tables view.',
  ...
  return (
    <section id="all-projects" className="all-projects">
      <div className="container">
        ...
        {showFeaturedModal && (
          <div className="about-featured-modal-backdrop" onClick={() => {
            if (videoRef.current) {
              try { videoRef.current.pause(); videoRef.current.currentTime = 0; } catch (e) {}
            }
            setShowFeaturedModal(false);
            ...
            <div className="about-featured-modal" role="dialog" aria-label="BlogShark featured post" onClick={(e) => e.stopPropagation()}>
              <div className="about-featured-modal-header">
                ...
                <button className="about-featured-close" onClick={() => { if (videoRef.current) { try { videoRef.current.pause(); videoRef.current.currentTime = 0; } catch (e) {} } setShowFeaturedModal(false); }} aria-label="Close preview">✕</button>
              </div>
              ...
              <div className="about-featured-actions">
                <a href="https://www.instagram.com/p/DUVf_PRDOQG/" target="_blank" rel="noopener noreferrer" className="about-featured-link">Show on Instagram</a>
              </div>
            </div>
          </div>
        )}
